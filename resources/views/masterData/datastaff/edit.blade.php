...
<link href="{{asset("admin/demo/plugins/bootstrap-datepicker/bootstrap-datepicker.min.css")}}" rel="stylesheet">
<!--Bootstrap Select [ OPTIONAL ]-->
<link href="{{asset("admin/demo/plugins/bootstrap-select/bootstrap-select.min.css")}}" rel="stylesheet">
<div class="boxed">
    <!--CONTENT CONTAINER-->
    <!--===================================================-->
        ...
        <div id="page-content">
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    <form class="form-horizontal" action="/data/staff/{{$staff->id}}" method="POST">
                        @csrf
                        @method('put')
                        <div class="panel-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea id="textarea-input-live_at" rows="2" class="form-control"
                                            placeholder="Alamat Lengkap" name="live_at"
                                            >{{$staff->live_at}}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        <div class="radio">
                                            ...
                                            <input id="employee_status_radio-1" class="magic-radio" type="radio"
                                                name="employee_status" value="Tetap" onclick="showContractOption()" {{$staff->employee_status == 'Tetap' ? 'checked' : ''}}>
                                            <label for="employee_status_radio-1">Tetap</label>
                                            <input id="employee_status_radio-2" class="magic-radio" type="radio"
                                                name="employee_status" value="Kontrak" onclick="showContractOption()" {{$staff->employee_status == 'Kontrak' ? 'checked' : ''}}>
                                    ...
                                    <span id="input-contract_duration">
                                        ...
                                        <div class="col-sm-4">
                                            <input type="text" placeholder="Lama kontrak dalam satuan bulan"
                                                ...
                                                class="form-control @error('contract_duration') is-invalid @enderror" value="{{$staff->contract_duration}}">
                                            ...
                                            <small class="text-muted">Tidak perlu diisi jika staff berstatus tetap.</small>
                                        </div>
                                    </span>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <div class="radio">
                                        <!-- Inline radio buttons -->
                                        <input id="employee_type_radio-1" class="magic-radio" type="radio"
                                            name="employee_type" value="Fulltime" {{$staff->employee_type == 'Fulltime' ? 'checked' : ''}}>
                                        <label for="employee_type_radio-1">Fulltime</label>
                                        <input id="employee_type_radio-2" class="magic-radio" type="radio"
                                            name="employee_type" value="Freelance" {{$staff->employee_type == 'Freelance' ? 'checked' : ''}}>
                                        <label for="employee_type_radio-2">Freelance</label>
                                        <input id="employee_type_radio-3" class="magic-radio" type="radio"
                                            name="employee_type" value="Magang" {{$staff->employee_type == 'Magang' ? 'checked' : ''}}>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <div class="radio">
                                        <!-- Inline radio buttons -->
                                        <input id="status_radio-1" class="magic-radio" type="radio" name="status"
                                            value="Aktif" {{$staff->status == 'Aktif' ? 'checked' : ''}}>
                                        <label for="status_radio-1">Aktif</label>
                                        <input id="status_radio-2" class="magic-radio" type="radio" name="status"
                                            value="Non-Aktif" {{$staff->status == 'Non-Aktif' ? 'checked' : ''}}>
                                ...
                                <div class="row">
                                    ...
                                    <div id="datepicker-edit-selesai-kerja">
                                        <div class="col-sm-4">
                                            <div class="input-group date">
                                                <input type="text"
                                                    class="form-control @error('end_work_date') is-invalid @enderror"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-4">
                                        <select class="selectpicker" data-style="btn-info" name="division_id">
                                            @foreach ($divisions as $item)
                                                <option value="{{$item->divisions_id}}" {{$staff->division_id == $item->divisions_id ? 'selected' : ''}}>{{$item->divisions_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-sm-4">
                                        <select class="selectpicker" data-style="btn-success" name="position_id">
                                            ...
                                            <option value="{{$item->positions_id}}" {{ $staff->position_id == $item->positions_id ? 'selected' : ''}}>{{$item->positions_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-4">
                                        <select class="selectpicker" data-style="btn-purple" name="role_id">
                                            ...
                                            <option value="{{$item->roles_id}}"{{ $staff->role_id == $item->roles_id ? 'selected' : '' }}>{{$item->roles_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-sm-4">
                                        <select class="selectpicker" data-style="btn-pink" name="shift_id">
                                            ...
                                            <option value="{{$item->shifts_id}}"{{ $staff->shift_id == $item->shifts_id ? 'selected' : '' }}>{{$item->shifts_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <input type="text" placeholder="Sisa cuti diisi hanya dengan angka"
                                        ...
                                        class="form-control @error('yearly_leave_remaining') is-invalid @enderror" value="{{$staff->yearly_leave_remaining}}">
    ...
    <!--Bootstrap Timepicker [ OPTIONAL ]-->
    <script src="{{asset("admin/demo/plugins/bootstrap-datepicker/bootstrap-datepicker.min.js")}}"></script>
    <!--Bootstrap Select [ OPTIONAL ]-->
    <script src="{{asset("admin/demo/plugins/bootstrap-select/bootstrap-select.min.js")}}"></script>
